...
int main(int argc, char *argv[])
{
	...
	if (rank == 0)
	{
		...
		//print(matriz, N);
		MPI_Send(matriz, N, MPI_FLOAT, 1, TAG, MPI_COMM_WORLD);
		// MPI_Send(matriz, N, MPI_FLOAT, 2, TAG, MPI_COMM_WORLD);
		// MPI_Send(matriz, N, MPI_FLOAT, 3, TAG, MPI_COMM_WORLD);
	}
	else
	{
		MPI_Recv(matriz, N, MPI_FLOAT, 0, TAG, MPI_COMM_WORLD, &status);
